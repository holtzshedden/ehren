...
export async function POST(_r:Request,{params}:{params:Promise<{id:string}>}){
 try{
  ...
  if(d.warehouse_movement_id){
   ...
   if(m?.product_id&&m?.source_location_id){const[x]=await sql`INSERT INTO warehouse_movements(movement_date,movement_type,product_id,batch_id,quantity,unit_cost,destination_location_id,cost_center_id,note,created_by_clerk_user_id,created_by_name) VALUES(${today},'IN',${m.product_id},${m.batch_id||null},${m.quantity},${m.unit_cost||0},${m.source_location_id},${m.cost_center_id||null},${`Gutschrift ${d.document_number}`},${userId},${name}) RETURNING id`; returnMovementId=x.id;}
  }
