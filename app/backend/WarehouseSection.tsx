...
const td=()=>new Date(Date.now()-new Date().getTimezoneOffset()*60000).toISOString().slice(0,10); const f=(n:number,d=2)=>new Intl.NumberFormat("de-DE",{minimumFractionDigits:d,maximumFractionDigits:d}).format(n); const ln=(x:string)=>({IN:"Wareneingang",OUT:"Ausbuchung",SALE:"Verkauf",TRANSFER:"Transfer",LOSS:"Bruch / Verlust",RETURN:"Gutschrift / Rückgabe"} as any)[x]||x;
...
<div className="backend-section-head">
  ...
  <div className="warehouse-filter-dummy">
    {["ALL","IN","OUT","SALE","RETURN","TRANSFER","LOSS"].map(v=>
      ...
      <tr key={r.id}>
        ...
        <td className="warehouse-route-cell">
          <div className="warehouse-route">
            ...
            <span
              className="warehouse-route-name"
              title={["IN","RETURN"].includes(r.movement_type)?r.destination_location||"–":r.destination_name||r.destination_location||"–"}
            >
              {["IN","RETURN"].includes(r.movement_type)?r.destination_location||"–":r.destination_name||r.destination_location||"–"}
            </span>
          </div>
        </td>
        <td>{r.user_name||"–"}</td>
